...
public class InventoryManager : MonoBehaviour
{
    ...
    [SerializeField] List<ObjectType> spawnPinObjectTypes; //룩 룩2 비숍 비숍2 퀸은4개다모이면 퀸
    ...
    private void Awake()
    {
        for (int i = 4; i < 8; i++)
        {
            spawnPinObjectTypes.Add(PlayerInfo.instance.pinobjectTypes[i]);
        }
    ...
    public List<GameObject> ponDList = new List<GameObject>();
    ...
    void RankUp()
    {
        ...
        for (int i = 0; i < pinList.Count; i++)
        {
            ...
            switch (pinList[i].GetComponent<MouseDrag>().objectType.atkType)
            {
                ...
                case AtkType.PonC:
                    k = 3;
                    RankUpSys(pinList, ponCList, i, k);
                    break;
                case AtkType.PonD:
                    k = 2;
                    RankUpSys(pinList, ponDList, i, k);
                    break;
                case AtkType.Lock:
                    break;
                case AtkType.Bishop:
